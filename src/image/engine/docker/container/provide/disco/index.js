...
var DiscoDependencyBase = require('../../../../common/container/common/disco');
...
Provision.prototype.onContainerStarted = function (steps, callback, container) {
    ...
    if ('eth0' == container.env.getNetworkInternal().name) {
        ...
        this.discoId = this.getDiscoClient(container).addProvision(
            this.ccontainer.get('name.endpoint'),
            {
                address : container.env.getNetworkExternal().address,
                port : this.cimage.get('port')
            },
            {
